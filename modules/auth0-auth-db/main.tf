data "auth0_connection" "my-database-connection" {
  name = var.name
}
...
resource "auth0_connection_clients" "db_clients_assoc" {
  connection_id   = data.auth0_connection.my-database-connection.id
  enabled_clients = var.enabled_clients
}